...
void Tiles2d::draw(const float x, const float y, const float width, const float height, const bool shouldBind) const {
    ...
    const float nx = window->sX(gx) + window->sX(0) * (gx / Window::GAME_WIDTH) + static_cast<float>(m_tilesHoriz) / width;
    const float ny = window->sY(gy) + window->sY(0) * (gy / Window::GAME_HEIGHT) + static_cast<float>(m_tilesVert) / height;
...
void Tiles2d::drawRaw(const float x, const float y, const float width, const float height, const bool shouldBind) const {
    ...
    if (shouldBind) tiles2dShader->setInt("width", m_tilesHoriz);
    if (shouldBind) tiles2dShader->setInt("height", m_tilesVert);
    if (shouldBind) tiles2dShader->setInt("textureTiles", m_textureTileCount);
    if (shouldBind) tiles2dShader->set8BitUnsignedIntArray("tiles", m_data, m_tilesHoriz * m_tilesVert);
