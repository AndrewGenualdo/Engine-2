...
MultiTexture2d Player::texture = MultiTexture2d();
...
Player::Player(Window *w, const std::string &tex, float scale) {
    ...
    if (texture.getPath().empty()) {
        anims[TEX_RIGHT] = Animation(tex + "right.png", 3, 0.1f, true);
        texture = MultiTexture2d(tex + "still.png", 1);
        anims[TEX_LEFT] = Animation(tex + "left.png", 3, 0.1f, true);
        anims[TEX_STILL] = Animation(tex + "still.png", 1, 1.0f, false);
        anims[TEX_MOUSE_GRABBED] = Animation(tex + "mouse_drag.png", 8, 0.05f, true);
        anims[TEX_SIT] = Animation(tex + "sit/", 4, 0.02f, false);
        anims[TEX_SLIDE_LEFT] = Animation(tex + "slide_left.png", 3, 0.05f, false);
        anims[TEX_SLIDE_RIGHT] = Animation(tex + "slide_right.png", 3, 0.05f, false);
        anims[TEX_WALL_SLIDE_LEFT] = Animation(tex + "wall_slide_left.png", 1, 1.0f, false);
        anims[TEX_WALL_SLIDE_RIGHT] = Animation(tex + "wall_slide_right.png", 1, 1.0f, false);
        anims[TEX_ROCKET_CHARGE] = Animation(tex + "rocket_charge.png", 4, maxRocketChargeDuration / 4.0f, false);
    }
...
int Player::bind() const {
    if (keys[MOUSE_LEFT]) return anims[TEX_ROCKET_CHARGE].bindSingle(window->getTime(), animStart);
    else if (sitting && velocity.x > startSpeed) return anims[TEX_SLIDE_RIGHT].bindSingle(window->getTime(), animStart);
    else if (sitting && velocity.x < -startSpeed) return anims[TEX_SLIDE_LEFT].bindSingle(window->getTime(), animStart);
    else if(wallSlide && facing > 0) return anims[TEX_WALL_SLIDE_LEFT].bindLoop(window->getTime());
    else if(wallSlide && facing < 0) return anims[TEX_WALL_SLIDE_RIGHT].bindLoop(window->getTime());
    else if (facing > 0) return anims[TEX_RIGHT].bindLoop(window->getTime());
    else if (facing < 0) return anims[TEX_LEFT].bindLoop(window->getTime());
    else if (sitting) return anims[TEX_SIT].bindSingle(window->getTime(), animStart);
    else return anims[TEX_STILL].bindLoop(window->getTime());
}
...
void Player::draw() const {
    ...
    if(keys[MOUSE_LEFT]) {
        ...
        texture.draw(nx, ny + 41 * scale, 90 * scale, 90 * scale, angle, frame, false);
    } else {
        texture.draw(nx, ny + 41 * scale, 90 * scale, 90 * scale, frame, false);
    }
